...
### 기존 nginx_proxy 연동 (HTTPS) — DuckDNS 미사용, sslip.io 사용
프론트가 호스트 `5555`로 공개되므로, **기존 `nginx_proxy` 컨테이너에 서버 블록 하나만 추가**해
**`13.125.173.69.sslip.io` → `호스트:5555`**로 프록시한다. (DuckDNS는 쓰지 않음 — sslip.io는 IP 기반이라 등록 불필요)
정확한 경로/인증서 방식은 기존 설정을 보고 맞춘다:
```bash
docker inspect nginx_proxy --format '{{json .Mounts}}'   # 설정/인증서 호스트 경로
```
- `nginx_proxy` 설정에 server 블록 추가(`server_name 13.125.173.69.sslip.io;`, `proxy_pass`는 기존 사이트와 동일한 호스트 게이트웨이 방식)
- 인증서: `sslip.io` 호스트네임으로 Let's Encrypt 발급(기존 cert 체계 재사용; HTTP-01 챌린지는 nginx_proxy가 80을 점유하므로 그 컨테이너를 통해 처리) → reload.
