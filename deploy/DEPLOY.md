...
```bash
# 1) 테니스 컨테이너를 공용 네트워크에 연결
docker network connect usuniverse_app-network tennis-frontend
docker network connect usuniverse_app-network tennis-backend

# 2) nginx 설정 파일에 테니스용 80/443 server 블록 추가
#    (deploy/nginx-tennismanager-block.conf 내용을 http{} 안, 기존 블록과 나란히)
#    파일: /home/ubuntu/UsUniverse/nginx/nginx.conf

# 3) 인증서 발급(기존 certbot webroot 재사용)
docker run --rm \
  -v /home/ubuntu/UsUniverse/certbot/conf:/etc/letsencrypt \
  -v /home/ubuntu/UsUniverse/certbot/www:/var/www/certbot \
  certbot/certbot certonly --webroot -w /var/www/certbot \
  -d 13.125.173.69.sslip.io --email you@example.com --agree-tos --no-eff-email

# 4) 재시작 전 임시 컨테이너로 새 설정 안전 검증
docker run --rm \
